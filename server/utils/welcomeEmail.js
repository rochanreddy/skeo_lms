...
const firstNameOf = (fullName, email) => String(fullName || '').trim().split(/\s+/)[0]
  || String(email || '').split('@')[0] || 'there';

const SUPPORT = 'support@skeoai.com';

const P = (inner, top = 18) => `<p style="margin:${top}px 0 0; font-size:15px; line-height:1.7; color:#23202e;">${inner}</p>`;
 ...
 */
export function welcomeEmail({ fullName, email, password = null, loginUrl, batches = [], allAccess = false }) {
    ...
    <tr><td align="center" style="padding:28px 12px;">
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" border="0" style="width:100%; max-width:560px; background:#ffffff; border-radius:14px; overflow:hidden;">
        ...
        <tr><td align="center" style="padding:24px 32px 8px;">
          <a href="${esc(loginUrl)}" style="display:inline-block; padding:13px 34px; background:#15121f; color:#ffffff; font-size:15px; font-weight:700; text-decoration:none; border-radius:8px;">Sign in to skeo</a>
        </td></tr>
